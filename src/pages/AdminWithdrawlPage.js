...
const AdminWithdrawl = (props) => {
  ...
  return (
    <Page title={(<><Link to="/my"><Typography type="h4" className={bem.e('title')}><FaArrowCircleLeft /> Admin Withdrawl</Typography></Link></>)} className="MyPage"
    >
      <Row>
        <Col md={12}>
        ...
        <Col md={12}>
          {
            (list && list.length > 0) ?
              list.map((ele, key) => (
                <Button tag="a" className={'form-control'} color="link" onClick={toggle(key)} key={key} style={{ marginBottom: '0rem' }}>{ele.userPhone}, {ele.userNickname} : ₹ {ele.money}
